...
# --- 3. PDF READING & STATS EXTRACTION ---
def open_and_read_pdf(pdf_path: str) -> list[dict]:
    ...
    for page_number, page in tqdm(enumerate(doc)):
        ...
        pages_and_texts.append({
            "page_number": page_number - 41, # adjust page numbers based on PDF start
            "page_char_count": len(text),
            "page_word_count": len(text.split(" ")),
            "page_sentence_count_raw": len(text.split(". ")),
            "page_token_count": len(text) / 4, # 1 token is ~4 chars
            "text": text
        })
...
print(pages_and_texts[:2]) # Display first two pages

import pandas as pd

# importing the list of dicts into a dataframe converts it into a table
...
# Load once globally
# 6 layers, based on MiniLM (BERT)
# maps text to a 384-dimensional dense vector space
# truncates input text longer than 256 word pieces (tokens)
...
# all-mpnet-base-v2 (768-dimensional vectors, 420MB, 384 word input limit) highest embedding quality amongst sentence transformer model;5 times slower than all-MiniLM-L6-v2 (384-dimensional vectors, 90MB, 256 word input limit)
# Choose all-mpnet-base-v2 when accuracy is the priority over inference speed
...
# (skipped - batched encoding below is faster and produces the same result)
...
if os.path.exists(embeddings_df_save_path):
    print(f"Embeddings file found at '{embeddings_df_save_path}', skipping embedding creation.")
else:
    # Turn text chunks into a single list
    text_chunks = [item["sentence_chunk"] for item in pages_and_chunks_over_min_token_len]

    print(f"Number of chunks to embed: {len(text_chunks)}")
    ...
    # Add embeddings back to each chunk dict
    for item, embedding in zip(pages_and_chunks_over_min_token_len, text_chunk_embeddings):
...
# ~0.00008 seconds to perform a dot product comparison across 1680 embeddings on my machine
...
# We've covered an example of using embedding vector search to find relevant results based on a query.
...
if torch.cuda.is_available():
    ...
    import psutil
    gpu_memory_gb = round(psutil.virtual_memory().total / (2**30))
    print(f"Running on Apple MPS, total system memory: {gpu_memory_gb} GB")
else:
    gpu_memory_gb = 0
...
# Stream tokens to console so we can see progress
...
# OPENAI_API_KEY already loaded from .env via load_dotenv() above
...
# * Try different prompts (e.g. see prompting techniques online). Prompt engineering resources for more prompting techniques - promptinguide.ai, Brex's Prompt Engineering Guide
...
# * Libraries/frameworks such as LangChain / LlamaIndex can help do many of the steps for you - so it's worth looking into those next
